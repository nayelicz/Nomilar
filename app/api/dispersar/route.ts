...
/** En Vercel: aumenta el lÌmite de timeout para operaciones de dispersiÛn en Avalanche (plan Hobby permite hasta 60s). */
 ...
 * - NEXT_PUBLIC_DISPERSOR_API_URL: alternativa si ya la ten√≠as; tambi√©n la usa el proxy
 */
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(
      {
        error: `No se pudo contactar al dispersor en ${base}. En Vercel configura DISPERSOR_API_URL con la URL p√∫blica del API. Detalle: ${msg}`,
      },
      { status: 502 }
    )
  }
